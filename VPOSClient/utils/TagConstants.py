def getBPWXmlRequestTag():
    return "BPWXmlRequest"


def getReleaseTag():
    return "Release"


def getRequestTag():
    return "Request"


def getOperationTag():
    return "Operation"


def getTimestampTag():
    return "Timestamp"


def getMACTag():
    return "MAC"


def getDataTag():
    return "Data"


def getHeaderTag():
    return "Header"


def getShopIDTag():
    return "ShopID"


def getOperatorIDTag():
    return "OperatorID"


def getReqRefNumTag():
    return "ReqRefNum"


def getData3DSTag():
    return "Data3DS"


def getServiceTag():
    return "Service"


def getEciTag():
    return "Eci"


def getXidTag():
    return "Xid"


def getCAVVTag():
    return "CAVV"


def getParesStatusTag():
    return "ParesStatus"


def getScEnrollStatusTag():
    return "ScEnrollStatus"


def getSignatureVerifytionTag():
    return "SignatureVerifytion"


def getMasterpassDataTag():
    return "MasterpassData"


def getPP_AuthenticateMethodTag():
    return "PP_AuthenticateMethod"


def getPP_CardEnrollMethodTag():
    return "PP_CardEnrollMethod"


def getOrderIDTag():
    return "OrderID"


def getOrderIdTag():
    return "OrderId"


def getPanTag():
    return "Pan"


def getCVV2Tag():
    return "CVV2"


def getExpDateTag():
    return "ExpDate"


def getAmountTag():
    return "Amount"


def getCurrencyTag():
    return "Currency"


def getExponentTag():
    return "Exponent"


def getAccountingModeTag():
    return "AccountingMode"


def getNetworkTag():
    return "Network"


def getEmailCHTag():
    return "EmailCH"


def getNameCHTag():
    return "NameCH"


def getUseridTag():
    return "Userid"


def getProductRefTag():
    return "ProductRef"


def getNameTag():
    return "Name"


def getSurnameTag():
    return "Surname"


def getOriginalReqRefNumTag():
    return "OriginalReqRefNum"


def getPaResTag():
    return "PaRes"


def getAcquirerTag():
    return "Acquirer"


def getProductRefTag():
    return "ProductRef"


def getTransactionIDTag():
    return "TransactionID"


def getOpDescrTag():
    return "OpDescr"


def getOptionsTag():
    return "Options"


def getTaxIDTag():
    return "TaxID"


def getTRecurrTag():
    return "TRECURR"


def getCRecurrTag():
    return "CRECURR"


def getInstallmentsNumberTag():
    return "INSTALLMENTSNUMBER"


def getInPersonTag():
    return "InPerson"


def getMerchantURLTag():
    return "MerchantURL"


def getOrderStatusTag():
    return "OrderStatus"


def getVerifyRequestTag():
    return "VerifyRequest"


def getRefundRequestTag():
    return "Refund"


def getConfirmRequestTag():
    return 'Accounting'


def getConfirmPaymentRequestTag():
    return 'DeferredRequest'


def getCloseOrderTag():
    return 'CloseOrder'


def getThreeDSAuthorizationRequest0Tag():
    return 'ThreeDSAuthorizationRequest0'


def getThreeDSAuthorizationRequest1Tag():
    return 'ThreeDSAuthorizationRequest1'


def getThreeDSAuthorizationRequest2Tag():
    return 'ThreeDSAuthorizationRequest2'


def getAuthorization3DSRequestTag():
    return 'AuthorizationRequest'


def getAuthorization3DSStep2RequestTag():
    return 'Authorization3DS'


def getIpAddressTag():
    return 'IpAddressTag'


def getUserAuthFlagTag():
    return 'UsrAuthFlag'


def getAntiFraudTag():
    return 'Antifraud'


def getCreatePanAliasTag():
    return 'CreatePanAlias'


def getThreeDSDataTag():
    return 'ThreeDSData'


def getThreeDSMtdTag():
    return 'ThreeDSMethod'


def getNotifUrLTag():
    return 'NotifUrl'


def getCprofTag():
    return 'Cprof'


def getThreeDSMtdNotifUrl():
    return 'ThreeDSMtdNotifUrl'


def getChallengeWinSizeTag():
    return 'ChallengeWinSize'


def getParesTag():
    return 'PaRes'


def getResultTag():
    return 'Result'


def getStatusTag():
    return 'Status'


def getSrcTypeTag():
    return 'SrcType'


def getTimestampElabTag():
    return 'TimestampElab'


def getTimestampReqTag():
    return 'TimestampReq'


def getTransactionAmountTag():
    return 'TransactionAmount'


def getAccountedAmountTag():
    return 'AccountedAmount'


def getRefundedAmountTag():
    return 'RefundedAmount'


def getTransactionResultTag():
    return 'TransactionResult'


def getAuthorizationNumberTag():
    return 'AuthorizationNumber'


def getAcquirerBinTag():
    return 'AcquirerBIN'


def getMerchantIdTag():
    return 'MerchantID'


def getTransactionStatusTag():
    return 'TransactionStatus'


def getResponseCodeIsoTag():
    return 'ResponseCodeISO'


def getPanTailTag():
    return 'PanTail'


def getPanAliasTag():
    return 'PanAlias'


def getPanExpiryDateTag():
    return 'PanExpiryDate'


def getPanAliasTailTag():
    return 'PanAliasTail'


def getPaymentTypePPTag():
    return 'PaymentTypePP'


def getThreeDSTransactionIDTag():
    return 'ThreeDSTransId'


def getThreeDSMtdComplIndTag():
    return 'ThreeDSMtdComplInd'


def getACSUrlTag():
    return 'URLAcs'


def getThreeDSChallengeTag():
    return 'ThreeDSChallenge'


def getCreqTag():
    return 'CReq'


def getThreeDSMtdUrlTag():
    return 'ThreeDSMethodUrl'


def getThreeDSMtdDataTag():
    return 'ThreeDSMethodData'


def getRRNTag():
    return 'RRN'


def getCardType():
    return 'CardType'


def getAuthorizationTypeTag():
    return 'AuthorizationType'


def getAuthorizationTag():
    return 'Authorization'


def getAuthorizedAmountTag():
    return 'AuthorizedAmount'


def getPaymentTypeTag():
    return 'PaymentType'


def getPanAliasRevTag():
    return 'PanAliasRev'


def getPanAliasDataTag():
    return 'PanAliasData'


def getAuthorizationRequestTag():
    return 'AuthorizationRequest'
