...
def build_get_html_payment_Request(shop_id, url_back, url_done, url_ms, amount, currency, exponent, order_id, accountingMode,
                 authorMode):
    request = PaymentRequestDto(shop_id, url_back, url_done, url_ms, amount, currency, exponent, order_id, accountingMode,
                 authorMode)
    request.options = "GM"
    return request
...
def build_confirm_transaction(transaction_id, amount, currency, order_id, shop_id, operator_id):
    return ConfirmTransactionRequestDto(transaction_id, amount, currency, order_id, shop_id, operator_id)


def build_refund_request(transaction_id, order_id, amount, currency, shop_id, operator_id):
    return RefundRequestDto(transaction_id, order_id, amount, currency, shop_id, operator_id)
