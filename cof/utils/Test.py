from cof.client.VPOSClient import VposClient
from cof.dto.RequestDto import *
from cof.utils.RequestBuilderForTest import *
from cof.utils.Utils import gen_order_id

shop_id = '129281292800109'
startKey="fU-9et-s-Sj8W---E8uhUDu9fEzqr8hH3L95s48r9nq-cq3cBXbp-tZsvGQU--t-nqmtaW-7x-7-C2PdcuFdbHuShQ-pYVWnr-4-"
apiResultKey = 'E-vmE-GHXmx73-Lfg24LztZ-7-yCyVsKn4QXphL5qzf-Kr-Cf-JWpZwZgaZRA5dR9V677xL4uCbc-Ce--8h2-tdrSu--QKjF-nZh'
orderId = '0oiujh6rd3tbhberwwww3g4ui574'
operatorId = 'Giammaicol'
pan = '5342230500001234'
expDate = '2112'
amount = '100'
currency = '978'
accountingMode = 'D'
network = '02'
urlBack= "http://localhost:8080/payment-gateway/vpos/tokenize"
urlDone= "http://localhost:8080/payment-gateway/vpos/tokenize"
urlMs= "https://te.t-frutta.eu/TImooneyWS/app_api/v10/payment/cardData?consumerId=3b350c34-d923-4552-91bf-67bc4f99da92"
amount = '10'
originalReqRefNum = '20200210770969248727528005362259'
pares = ''
base64_Html= ""

client = VposClient(startKey, apiResultKey)
client.set_proxy("http://proxy-dr.reply.it:8080")
client.injectTemplate(base64_Html, 1000)

print(client.getHtmlPaymentDocument(build_get_html_payment_Request(shop_id, urlBack, urlDone, urlMs, amount, currency, "2", gen_order_id(), accountingMode, "I")))

# client.getOrderStatus("15816791805478866964400948992201416592057357867885", None, shop_id, operatorId)
#client.confirmTransaction(build_confirm_transaction("8032112928AT1gx2zgaykxk74", amount, currency, "15816791805478866964400948992201416592057357867885", shop_id, operatorId))
#client.refundPayment(build_refund_request("8032112928AT1gx2zgaykxk74", "15816791805478866964400948992201416592057357867885", amount, currency, shop_id, operatorId))
#client.start3DSAuth(build_start_3DS_Auth_Request(shop_id, gen_order_id(), operatorId, expDate, amount, currency, accountingMode))

#client.start3DSAuthStep2(Auth3DSStep2RequestDto(shop_id, "15816949165842401964639248134781030919538750832490", operatorId, "20200214908528468312702620847941", pares))


